...
`BcAuthPasskey` と `BcAuthSocial` の共通ログイン完了処理・リダイレクト正規化・監査ログ・認証入口管理を実装しています。
...
各プラグインの `Plugin::bootstrap()` で登録する `'order'` 値で制御します。

| order | プラグイン | 表示内容 |
|---|---|---|
| 10 | BcAuthPasskey | パスキーでログイン |
| 20 | BcAuthSocial | Google / X ボタン群 |

数値が小さいほど上に表示されます。変更する場合は対象プラグインの `src/BcXxxxPlugin.php` の `order` 値を書き換えてください。
