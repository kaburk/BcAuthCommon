...
> **注意**: Admin ログイン画面のソーシャルログインボタンは現在 BcPasskeyAuth の template override 内に統合されており、`class_exists(ProviderAdapterRegistry::class)` で条件分岐している。BcSocialAuth が単体でインストールされても Admin ログイン画面にソーシャルボタンは表示されない（後述の残タスク参照）。
...
| # | 内容 | 対象 | 優先度 |
|---|---|---|---|
| 1 | **BcSocialAuth 単体動作対応**: BcPasskeyAuth なしで Admin ログイン画面にソーシャルボタンを表示するため、BcSocialAuth 自身の `templates/plugin/BcAdminThird/Admin/Users/login.php` override を追加する | BcSocialAuth | 高 |
| 2 | **BcSocialAuth: Front prefix 用 AuthController 作成**（login / callback を Front 向けに実装） | BcSocialAuth | 高 |
| 3 | **BcSocialAuth: Front ログイン画面へのソーシャルボタン統合**（template override または element 差し込み） | BcSocialAuth | 高（2の後） |
| 4 | **Docker e2e 動作確認**（管理画面から3プラグインをinstallして実際の認証フローを確認） | 全体 | 高 |
| 5 | **X provider 実運用検証**（token exchange が実環境で通るか） | BcSocialAuth | 中 |
| 6 | **BcPasskeyAuth install 後の登録導線補強**（install 完了画面→パスキー登録画面への案内追加） | BcPasskeyAuth | 中 |
| 7 | **Front パスキーボタンの URL 確認**（`data-challenge-url` / `data-login-url` の正確性確認と調整） | BcPasskeyAuth | 中 |
| 8 | **Front 側デザイン調整**（実運用テーマに合わせた表示調整） | BcPasskeyAuth | 低 |
| 9 | **監査ログ共通化**（ログイン成功・失敗・キャンセルの記録形式統一） | BcAuthCommon | 低 |
| 10 | **認証入口定義の共通化**（複数プラグイン同時有効時のボタン順序制御） | BcAuthCommon | 低 |

### 次の実装順

1. **BcSocialAuth 単体動作対応**（BcPasskeyAuth 非依存の Admin ログイン override）
2. **BcSocialAuth Front prefix 対応**（AuthController + routes）
3. **Front ログイン画面へのソーシャルボタン統合**
4. **Docker e2e 動作確認**（管理画面から3プラグインをinstall → 認証フロー確認）
5. X provider 実運用検証
